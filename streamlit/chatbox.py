...
def generate_reponse(prompt_input, email, passwd):
    # Hugging Face Login
    sign = Login(email, pssswd)
    cookies = sign.login()
    #Create ChatBot
...
# Generate a new respose if last message is not from assistant
if st.session_state.messages[-1]["role"] != "assistant":
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            response = generate_response(prompt, hf_mail, hf_pass)
            st.write(response) 
